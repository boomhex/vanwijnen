...
class ComparisonPage(SubPage):
    def render(self):
        self.container = ui.column().classes('w-full h-full p-4')
        with self.container:
            self.show()
...
class RightSide(SubPage):
    def __init__(self, *, state: MainPageState, folder_handler: FolderHandler, pdf_dir: Path) -> None:
        super().__init__(state, folder_handler, pdf_dir)

        self.comparison_page = ComparisonPage(state, folder_handler, pdf_dir)
        self.offer_page = OfferPage(state, folder_handler, pdf_dir)

    def render(self) -> None:
        self.container = ui.column().classes('w-full h-screen max-h-screen overflow-hidden')
        with self.container:
            self.show()
